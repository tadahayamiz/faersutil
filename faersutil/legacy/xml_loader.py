...
class XMLoader():
    ...
    def load_xml(self,url,to_pickle=False,to_csv=False):
        """
        ...
        """
        return self.__load.load_xml(url,to_pickle=to_pickle,to_csv=to_csv)
...
class Loader2014Q3_():
    ...
    def load_xml(self,url,to_pickle=False,to_csv=False):
        ...
        candi_list = ['Physician','Pharmacist','Other Health Professional','Lawyer','Consumer or non-health proffesional']
        for v in saftyreports:
        ...
        #export data
        data = pd.DataFrame({'Case ID':caseID,'Active Substances':drugs,
                        'Reactions':reactions,'Sex':sex,'Event Date':event_date,'Patient Age':age,'Qualification':qualifications})
        del tree,root,event_country,event_date,age,sex,caseID,drugs,saftyreports,reactions,qualifications
        data = data.set_index("Case ID",drop=True).sort_index()
        
        if to_pickle:
            fileout = os.path.dirname(url) + '\\' + os.path.basename(url).replace('xml','pkl')
            data.to_pickle(fileout)
        elif to_csv:
            fileout = os.path.dirname(url) + '\\' + os.path.basename(url).replace('xml','csv')    
...
class Loader2012Q4_2014Q2():
    ...
    def load_xml(url,to_pickle=False,to_csv=False):
        """
        ...
        #export data
        data = pd.DataFrame({'Case ID':caseID,'Active Substances':drugs,
                        'Reactions':reactions,'Sex':sex,'Event Date':event_date,'Patient Age':age})
